...
const CodeUiSettingsSchema = z.object({
  ...
  // 切换按钮样式：header(会话头 ⌨ 按钮,默认) / bar(悬浮模式条)
  toggle: z.string().default("header"),
});
// 解析后的当前值：settings 服务未挂载/未写入时保持默认
const selfSettings = { updateRegistry: REGISTRY_DEFAULT, enabled: true, theme: "dsh", layout: "dock", uiMode: "native", toggle: "header" };
