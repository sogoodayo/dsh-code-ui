...
const selfSettings = { updateRegistry: REGISTRY_DEFAULT, enabled: true };
// settings 服务引用（「启用/禁用」开关写入用；由 installSelfSettings 捕获）
let settingsServiceRef = null;
function installSelfSettings(ctx) {
  let source = () => ({ updateRegistry: selfSettings.updateRegistry, enabled: selfSettings.enabled });
  installSettingsSection(
    ctx,
    CODE_UI_SETTINGS_NS,
    CodeUiSettingsSchema,
    { updateRegistry: selfSettings.updateRegistry, enabled: selfSettings.enabled },
    {
