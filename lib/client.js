...
// --- begin original apply body ---
    ...
    let toggleStyle = 'header' // header(会话头 ⌨ 按钮,默认) / bar(悬浮模式条)：用哪种按钮切换界面
    ...
    // 启动时先查开关/主题/布局/界面模式/切换按钮再挂载（RPC 失败 = 全默认：启用+跟随DSH+停靠+显示+会话头按钮）
    callHost('get-self').then((r) => {
      if (r && !r.error) {
        setThemeMode(r.theme)
        layoutMode = (r.layout === 'float') ? 'float' : 'dock'
        toggleStyle = (r.toggle === 'bar') ? 'bar' : 'header'
        applyUiMode(r.uiMode === 'native' ? 'native' : 'codeui')
      ...
      return h('li', { className: 'cdx-pc-card' + (cardOpen ? ' cdx-pc-card-open' : '') },
        ...
        cardOpen ? h('div', { className: 'cdx-pc-body' },
          ...
          h('div', { className: 'cdx-pc-field' },
            ...
            h('p', { className: 'cdx-pc-note' }, '「右侧停靠」code UI 固定占屏幕右侧、与聊天并排（默认）；「悬浮窗口」为可拖拽的浮动编辑器窗口、含自带输入框；「隐藏（dsh UI）」收起编辑器、回归 DSH 原生界面（⌨ / 模式条 / ✕ 随时可唤回，隐藏期间记住当前布局，切回原样恢复）。切换即时生效，持久化保存。'),
          ),
          // 字段：切换按钮
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              ...
              h('span', { className: 'cdx-pc-head' },
                ['header', 'bar'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.toggle) || 'header') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || toggling,
                  onClick: () => setToggle(m),
                }, m === 'header' ? '会话头按钮' : '悬浮模式条')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '选择用哪种按钮在 code UI 与dsh UI 之间切换：「会话头按钮」在会话标题栏显示 ⌨ code UI（默认）；「悬浮模式条」为屏幕上方可拖动的横条（code UI / dsh UI 两键）。与布局模式无关，可任意组合，选择持久化保存。'),
          ),
          // 字段一：在线检查更新
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
