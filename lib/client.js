...
// --- begin original apply body ---
    ...
    function PluginConfigCard() {
      ...
      const [layouting, setLayouting] = React.useState(false)
      const [uiModeing, setUiModeing] = React.useState(false)
      ...
      // 布局模式：写 host settings 持久化；成功后立即卸载旧挂载、按新模式重挂
      function setLayout(m) {
        if (!self || layouting) return
        setLayouting(true)
        callHost('set-layout', { layout: m }).then((r) => {
          setLayouting(false)
          if (r && !r.error) {
            setSelf(Object.assign({}, self, { layout: m }))
            applyLayout(m)
            showToast('布局已切换：' + (m === 'dock' ? '右侧停靠' : '悬浮窗口'))
          } else {
            showToast('切换失败：' + ((r && r.error) || '未知错误'))
          }
        })
      ...
      // 界面模式：写 host settings 持久化；成功后立即显示/隐藏编辑器（按当前布局）
      function setUiMode(m) {
        if (!self || uiModeing) return
        setUiModeing(true)
        callHost('set-uimode', { uiMode: m }).then((r) => {
          setUiModeing(false)
          if (r && !r.error) {
            setSelf(Object.assign({}, self, { uiMode: m }))
            applyUiMode(m)
            showToast('界面已切换：' + (m === 'codeui' ? 'code UI' : 'dsh UI'))
          } else {
            showToast('切换失败：' + ((r && r.error) || '未知错误'))
          }
        })
      ...
      return h('li', { className: 'cdx-pc-card' + (cardOpen ? ' cdx-pc-card-open' : '') },
        ...
        cardOpen ? h('div', { className: 'cdx-pc-body' },
          ...
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              h('span', { className: 'cdx-pc-label' }, '布局模式'),
              h('span', { className: 'cdx-pc-head' },
                ['dock', 'float'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.layout) || 'dock') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || layouting,
                  onClick: () => setLayout(m),
                }, m === 'dock' ? '右侧停靠' : '悬浮窗口')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '「右侧停靠」code UI 固定占屏幕右侧、与聊天并排，会话头 ⌨ 按钮开关（默认）；「悬浮窗口」为可拖拽的浮动编辑器窗口，盖住页面、含自带输入框。切换即时生效，选择持久化保存。'),
          ),
          // 字段：界面模式
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              h('span', { className: 'cdx-pc-label' }, '界面模式'),
              h('span', { className: 'cdx-pc-head' },
                ['codeui', 'native'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.uiMode) || 'codeui') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || uiModeing,
                  onClick: () => setUiMode(m),
                }, m === 'codeui' ? 'code UI' : 'dsh UI')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '「code UI」显示编辑器界面（默认）；「dsh UI」隐藏编辑器、回归 DSH 原生界面（入口按钮保留，随时切回）。悬浮模式条 / ⌨ / ✕ / ↩ 的每次切换都会同步到此处，选择持久化保存。'),
          ),
          // 字段：切换按钮
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
