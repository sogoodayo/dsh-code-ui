...
// --- begin original apply body ---
    ...
    ctx.effect(() => { const tag = document.createElement('style'); tag.setAttribute('data-plugin', 'dsh-code-ui'); tag.textContent = `
      ...
      /* 原生视图模式：填满 conversation.view 容器（非 fixed 悬浮） */
      .cdex-nativeview{position:relative;width:100%;height:100%;flex:1 1 0;min-height:0;display:flex;flex-direction:column;background:var(--dsw-alias-bg-base,#1e1e1e);color:var(--dsw-alias-label-primary,#d4d4d4);font-family:-apple-system,'Segoe UI',system-ui,sans-serif;font-size:13px;overflow:hidden;box-sizing:border-box;}
      ...
      /* 宿主 viewArea 默认随内容增高（聊天式整页滚动），文件树会跟着右侧内容滚走。
         挂载原生视图时强制切到定高模式（对齐 DSH 自身 composer-overlay 变体的做法），
         使左侧文件树固定、只有编辑器/文件树各自内部滚动。 */
      .wSkVaW_viewArea:has(.cdex-nativeview){flex:1 1 0 !important;min-height:0 !important;overflow:hidden !important;}
    ...
    /* 原生视图模式（feature/native-layout 预览）：注册为 conversation.view 的一个
       标签页（与 聊天/轨迹 并列），填满 DSH 中栏容器，不再是悬浮窗。
       无窗口 chrome（拖拽/缩放/还原），其余布局与悬浮工作台一致。 */
    function NativeView() {
      const st = useStore()
      React.useEffect(() => {
        refresh(); loadReferences()
      ...
      return h('div', { className: 'cdex-nativeview ' + themeClass() + ' ' + tokensClass() },
        h('div', { className: 'cdex-nativeview-top' },
          ...
          h('div', { className: 'cdex-top-spacer' }),
          h('span', { className: 'cdex-top-hint' }, '布局预览（无交互）'),
          h('button', { className: 'cdex-btn-ghost', title: '打开文件 (Ctrl+P)', onClick: openFilePicker }, '📂'),
        ),
        h('div', { className: 'cdex-main' },
          h(ActivityBar),
    ...
    function DockToggle() {
      ...
      return h('button', {
        ...
        onClick: () => { st.dockOpen = !st.dockOpen; emit() },
      }, '⌨ Code UI')
    }
    function DockPanel() {
      const st = useStore()
      ...
      return h('div', { className: 'cdex-dock ' + themeClass() + ' ' + tokensClass(), style: { width: st.dockW } },
        ...
        h('div', { className: 'cdex-nativeview-top' },
          ...
          h('div', { className: 'cdex-top-spacer' }),
          h('button', { className: 'cdex-btn-ghost', title: '打开文件 (Ctrl+P)', onClick: openFilePicker }, '📂'),
          h('button', { className: 'cdex-btn-ghost', title: '收起 Code UI', onClick: () => { st.dockOpen = false; emit() } }, '✕'),
        ),
        h('div', { className: 'cdex-main' },
          h(ActivityBar),
          st.sidebarCollapsed ? h('div', { className: 'cdex-sb-expand', title: '展开侧栏', onClick: () => { st.sidebarCollapsed = false; emit() } }, '»') : (st.view === 'files' ? h(FileTree) : h(ReferencesView)),
    ...
    function mountEditor() {
      ...
      // 右侧停靠模式（feature/native-layout）：会话头开关按钮 + shell.overlay 停靠面板
      disposers.push(slots.inject('conversation.session.header.actions', () => slots.register(
    ...
    // 启动时先查开关与主题再挂载（RPC 失败 = 默认启用 + 跟随 DSH）
    ...
    }).catch(() => { setThemeMode('dsh'); applyEnabled(true) })
