...
// --- begin original apply body ---
    ...
    ctx.effect(() => { const tag = document.createElement('style'); tag.setAttribute('data-plugin', 'dsh-code-ui'); tag.textContent = `
      ...
      .cdex-mode-tab.active{background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);}
      ...
      .cdex-abar-btn.active::before{content:'';position:absolute;left:0;top:8px;bottom:8px;width:2px;background:var(--cdex-accent-bg,#0f1115);border-radius:1px;}
      .cdex-abar-spacer{flex:1;}
      .cdex-sidebar{width:264px;background:var(--dsw-specific-sidebar-fill,#252526);display:flex;flex-direction:column;min-height:0;position:relative;}
      .cdex-sb-resize{position:absolute;right:-3px;top:0;bottom:0;width:7px;cursor:col-resize;z-index:5;}
      .cdex-sb-resize:hover{background:var(--dsw-alias-interactive-bg-hover,#3a3a41);}
      ...
      .cdex-sidebar-head > span:first-child{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0;}
      .cdex-sidebar-head > :last-child{flex:none;}
      /* 收起后的展开把手：贴在原侧栏位置，» 一眼可见 */
      .cdex-sb-expand{width:18px;flex:0 0 auto;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--dsw-alias-label-secondary,#9d9d9d);background:var(--dsw-specific-sidebar-fill,#252526);border-right:1px solid var(--dsw-alias-border-l1,#333333);font-size:15px;user-select:none;}
      .cdex-sb-expand:hover{color:var(--dsw-alias-label-primary,#d4d4d4);background:var(--dsw-alias-interactive-bg-hover,#3a3a41);}
      ...
      .cdex-tab.active{background:var(--dsw-alias-bg-base,#1e1e1e);color:var(--dsw-alias-label-primary,#ffffff);border-top:1px solid var(--cdex-accent-bg,#0f1115);}
      ...
      .cdx-tk-kw{color:var(--cdex-tk-kw,#c586c0);font-weight:600;}
      .cdx-tk-str{color:var(--cdex-tk-str,#ce9178);}
      .cdx-tk-com{color:var(--cdex-tk-com,#6a9955);font-style:italic;}
      .cdx-tk-num{color:var(--cdex-tk-num,#b5cea8);}
      .cdx-tk-fn{color:var(--cdex-tk-fn,#dcdcaa);}
      .cdx-tk-tag{color:#569cd6;}
      .cdx-tk-props{color:#9cdcfe;}
      .cdx-tk-attr{color:#dcdcaa;}
      .cdx-tk-punc{color:var(--cdex-tk-punc,#808080);}
      ...
      .cdex-statusbar{display:flex;align-items:center;height:24px;background:var(--dsw-alias-bg-overlay,#252526);color:var(--dsw-alias-label-secondary,#9d9d9d);font-size:12px;padding:0 12px;gap:16px;user-select:none;border-top:1px solid var(--dsw-alias-border-l1,#333333);}
      .cdex-statusbar .cdex-dirty{color:var(--dsw-alias-state-warn-primary,#ffe082);font-weight:600;}
      .cdex-btn{border:1px solid var(--dsw-alias-border-l2,#3e3e42);background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);border-radius:4px;padding:4px 10px;cursor:pointer;font-size:12px;}
      ...
      .cdex-menu{position:fixed;z-index:10002;background:var(--dsw-alias-bg-overlay,#252526);border:1px solid var(--dsw-alias-border-l2,#454545);border-radius:6px;padding:4px;min-width:180px;box-shadow:0 8px 24px rgba(0,0,0,.5);max-height:calc(100vh - 24px);overflow-y:auto;}
      ...
      .cdex-menu-item:hover{background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);}
      ...
      .cdex-input{width:100%;background:var(--dsw-alias-bg-base,#1e1e1e);color:var(--dsw-alias-label-primary,#d4d4d4);border:1px solid var(--dsw-alias-border-l1,#3e3e42);border-radius:4px;padding:8px 10px;font-family:inherit;font-size:13px;box-sizing:border-box;outline:none;}
      .cdex-input:focus{border-color:var(--dsw-alias-label-dimmed,#6a6a70);}
      .cdex-confirm-msg{color:var(--dsw-alias-label-primary,#d4d4d4);font-size:13px;line-height:1.6;white-space:pre-wrap;word-break:break-word;}
      .cdex-btn-danger{background:var(--dsw-alias-state-error-primary,#f14c4c);color:#ffffff;border-color:transparent;}
      .cdex-btn-danger:hover{opacity:.9;}
      ...
      .cdex-toast{position:fixed;z-index:10003;top:36px;left:50%;transform:translateX(-50%);background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);padding:9px 16px;border-radius:6px;box-shadow:0 4px 16px rgba(0,0,0,.4);max-width:80vw;cursor:grab;user-select:none;touch-action:none;}
      .cdex-toast.placed{transform:none;cursor:grabbing;}
      .cdex-toast-grip{margin-right:8px;opacity:.55;font-size:12px;letter-spacing:-1px;}
      ...
      .cdex-aiinput-tag{display:inline-flex;align-items:center;gap:6px;background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);border-radius:12px;padding:3px 10px 3px 12px;font-size:12px;}
      .cdex-aiinput-tag-label{white-space:nowrap;}
      .cdex-aiinput-tag-tr{background:#b45309;color:#fff;}
      .cdex-aiinput-tag-x{border:none;background:transparent;color:var(--cdex-accent-fg,#ffffff);cursor:pointer;font-size:13px;padding:0;line-height:1;opacity:.85;}
      ...
      .cdex-aiinput-send{flex:0 0 auto;height:36px;min-width:76px;border:none;background:var(--cdex-accent-bg,#0f1115);color:var(--cdex-accent-fg,#ffffff);border-radius:999px;padding:0 18px;cursor:pointer;font-size:13px;font-weight:600;transition:filter .15s ease, opacity .15s ease;}
      ...
      /* —— 编辑器主题三模式：根节点局部重定义语义变量（仅编辑器生效，不影响 DSH 其他界面）—— */
      .cdex-theme-light{--dsw-alias-bg-base:#f8f8f8;--dsw-alias-bg-layer-1:#f0f0f0;--dsw-alias-bg-layer-2:#f5f5f5;--dsw-alias-bg-layer-3:#ffffff;--dsw-alias-bg-module-platform:#eff1f3;--dsw-alias-bg-overlay:#ffffff;--dsw-alias-border-l1:#e5e5e5;--dsw-alias-border-l2:#d6d6d6;--dsw-alias-brand-primary:#0e639c;--dsw-alias-interactive-bg-hover:#e9e9e9;--dsw-alias-label-primary:#1f2328;--dsw-alias-label-secondary:#59636e;--dsw-alias-label-tertiary:#8c959f;--dsw-alias-label-dimmed:#b6bec6;--dsw-alias-label-error:#cf222e;--dsw-alias-label-success:#1a7f37;--dsw-alias-state-error-primary:#cf222e;--dsw-alias-state-success-primary:#1a7f37;--dsw-alias-state-warn-primary:#9a6700;--dsw-specific-sidebar-fill:#f0f0f0;--cdex-accent-bg:#0f1115;--cdex-accent-fg:#ffffff;--cdex-tk-kw:#af00db;--cdex-tk-str:#a31515;--cdex-tk-com:#008000;--cdex-tk-num:#098658;--cdex-tk-fn:#795e26;--cdex-tk-punc:#555555;}
      .cdex-theme-dark{--dsw-alias-bg-base:#1e1e1e;--dsw-alias-bg-layer-1:#252526;--dsw-alias-bg-layer-2:#37373d;--dsw-alias-bg-layer-3:#2d2d2d;--dsw-alias-bg-module-platform:#3a3a41;--dsw-alias-bg-overlay:#252526;--dsw-alias-border-l1:#333333;--dsw-alias-border-l2:#3e3e42;--dsw-alias-brand-primary:#0e639c;--dsw-alias-interactive-bg-hover:#3a3a41;--dsw-alias-label-primary:#d4d4d4;--dsw-alias-label-secondary:#9d9d9d;--dsw-alias-label-tertiary:#8a8a8a;--dsw-alias-label-dimmed:#6a6a70;--dsw-alias-label-error:#f48771;--dsw-alias-label-success:#4ec9b0;--dsw-alias-state-error-primary:#f48771;--dsw-alias-state-success-primary:#4ec9b0;--dsw-alias-state-warn-primary:#ffe082;--dsw-specific-sidebar-fill:#252526;--cdex-accent-bg:#ffffff;--cdex-accent-fg:#0f1115;--cdex-tk-kw:#c586c0;--cdex-tk-str:#ce9178;--cdex-tk-com:#6a9955;--cdex-tk-num:#b5cea8;--cdex-tk-fn:#dcdcaa;--cdex-tk-punc:#808080;}
      /* 停靠面板顶栏（复用自原生视图预览） */
      .cdex-nativeview-top{display:flex;align-items:center;height:34px;flex:0 0 auto;background:var(--dsw-alias-bg-layer-2,#323233);padding:0 10px;gap:8px;user-select:none;border-bottom:1px solid var(--dsw-alias-border-l1,#333333);}
      /* 右侧停靠模式：固定占右缘，壳层帧被 margin-right 推挤 */
      .cdex-dock{position:fixed;top:0;right:0;bottom:0;z-index:9000;display:flex;flex-direction:column;background:var(--dsw-alias-bg-base,#1e1e1e);color:var(--dsw-alias-label-primary,#d4d4d4);font-family:-apple-system,'Segoe UI',system-ui,sans-serif;font-size:13px;overflow:hidden;box-sizing:border-box;border-left:1px solid var(--dsw-alias-border-l2,#3e3e42);}
      .cdex-dock .cdex-main{flex:1;min-height:0;}
      .cdex-dock-grip{position:absolute;left:0;top:0;bottom:0;width:7px;cursor:col-resize;z-index:10;}
      .cdex-dock-grip:hover{background:var(--dsw-alias-interactive-bg-hover,#3a3a41);}
      .cdex-dock-toggle{border:1px solid var(--dsw-alias-border-l2,#3e3e42);background:var(--dsw-alias-bg-layer-2,#37373d);color:var(--dsw-alias-label-secondary,#9d9d9d);border-radius:6px;padding:4px 10px;font-size:12px;cursor:pointer;white-space:nowrap;font-family:inherit;}
      .cdex-dock-toggle:hover{color:var(--dsw-alias-label-primary,#d4d4d4);}
      .cdex-dock-toggle.active{color:var(--dsw-alias-label-primary,#d4d4d4);border-color:var(--dsw-alias-label-dimmed,#6a6a70);}
      /* dsh 模式（不挂调色板）时的反色按钮与 token 色：
         反色按钮直接用 DSH 的 brand-primary（浅色=黑 / 深色=白），文字色取 bg-base（自动反色）；
         token 色由 tokensClass() 检测 body[data-ds-dark-theme] 后挂 .cdex-tokens-light/dark */
      .cdex-workbench,.cdex-modebar{--cdex-accent-bg:var(--dsw-alias-brand-primary,#0f1115);--cdex-accent-fg:var(--dsw-alias-bg-base,#ffffff);}
      .cdex-tokens-light{--cdex-tk-kw:#af00db;--cdex-tk-str:#a31515;--cdex-tk-com:#008000;--cdex-tk-num:#098658;--cdex-tk-fn:#795e26;--cdex-tk-punc:#555555;}
      .cdex-tokens-dark{--cdex-tk-kw:#c586c0;--cdex-tk-str:#ce9178;--cdex-tk-com:#6a9955;--cdex-tk-num:#b5cea8;--cdex-tk-fn:#dcdcaa;--cdex-tk-punc:#808080;}
    ...
    const store = {
      ...
      // 右侧停靠模式（feature/native-layout）：dockOpen 开关 + dockW 面板宽度（px）
      dockOpen: false,
      dockW: Math.max(380, Math.min(Math.round(vw * 0.46), vw - 480)),
      ...
      toastKeep: false,
      toastPos: null,
      sessionId: null,     // 当前会话 id（InputBridge 从 slot kit 捕获）：文件树默认根跟随当前会话
      rootIsAuto: true,    // 根是否为自动跟随（用户「以此文件夹为根」后置 false，不再跟随）
      rootSessionId: null, // 当前自动根来自哪个会话（落后于 sessionId 时触发重新解析）
      error: null,
    }
    ...
    function showToast(msg) {
      store.toast = { msg }
      store.toastKeep = false
      emit()
      scheduleToastHide(2800)
    }
    // 提示自动消失；正在被拖拽（toastKeep）时顺延，松手后短暂展示再消失
    function scheduleToastHide(ms) {
      if (timer && typeof timer.timeout === 'function') {
        timer.timeout(() => {
          if (store.toastKeep) { scheduleToastHide(600); return }
          store.toast = null; emit()
        }, ms)
      }
    }
    // 提示条拖拽：按住移动自定义位置（默认顶部居中，拖过之后记住位置，防止挡内容）
    function onToastPointerDown(e) {
      e.preventDefault()
      const el = e.currentTarget
      const rect = el.getBoundingClientRect()
      const base = store.toastPos || { x: rect.left, y: rect.top }
      const px = e.clientX, py = e.clientY
      store.toastKeep = true
      try { el.setPointerCapture(e.pointerId) } catch (err) {}
      function move(ev) {
        const vw = (typeof window !== 'undefined' && window.innerWidth) || 1200
        const vh = (typeof window !== 'undefined' && window.innerHeight) || 800
        store.toastPos = {
          x: Math.max(4, Math.min(base.x + (ev.clientX - px), vw - rect.width - 4)),
          y: Math.max(4, Math.min(base.y + (ev.clientY - py), vh - rect.height - 4)),
        }
        emit()
      }
      function up() {
    ...
    function Toast() {
      ...
      const style = st.toastPos ? { left: st.toastPos.x, top: st.toastPos.y } : null
      return h('div', { className: 'cdex-toast' + (st.toastPos ? ' placed' : ''), style: style, title: '按住可拖动位置（不再挡内容）', onPointerDown: onToastPointerDown },
        h('span', { className: 'cdex-toast-grip' }, '⠿'),
        st.toast.msg,
      )
    }

    /* ============ 编辑器主题四态：dsh(默认,跟随 DSH 宿主外观) / system(跟随 OS) / light / dark ============ */
    // 原理：dsh 模式不挂任何类，直接继承 DSH 的 --dsw-alias-* 变量（v1.1.7 前的原始行为）；
    // 其余模式在编辑器根节点挂 .cdex-theme-light / .cdex-theme-dark 类，类内局部重定义
    // 语义变量（CSS 自定义属性作用域），不影响 DSH 其他界面。
    // system 模式用 matchMedia 监听操作系统外观切换，实时换肤。
    let themeMode = 'dsh'
    let mqDispose = null
    let bodyObs = null
    ...
    // DSH 自身外观：body[data-ds-dark-theme] 由 DSH 主题系统维护
    function dshPrefersDark() {
      try { return !!(document.body && document.body.hasAttribute && document.body.hasAttribute('data-ds-dark-theme')) } catch (e) { return true }
    }
    function themeClass() {
      if (themeMode === 'light') return 'cdex-theme-light'
      if (themeMode === 'dark') return 'cdex-theme-dark'
      if (themeMode === 'system') return systemPrefersDark() ? 'cdex-theme-dark' : 'cdex-theme-light'
      return '' // dsh：继承宿主变量
    }
    // 语法 token 色类：dsh 模式按宿主当前外观选（palette 类自带 token 定义，无需另挂）
    function tokensClass() {
      if (themeMode === 'dsh') return dshPrefersDark() ? 'cdex-tokens-dark' : 'cdex-tokens-light'
      return ''
    }
    function setThemeMode(v) {
      themeMode = (v === 'light' || v === 'dark' || v === 'system') ? v : 'dsh'
      try {
        if (mqDispose) { mqDispose(); mqDispose = null }
        if (themeMode === 'system' && window.matchMedia) {
          ...
          if (typeof mq.addEventListener === 'function') {
            ...
            mqDispose = () => mq.removeListener(onChange)
          }
        }
        // dsh 模式监听宿主外观切换（token 色需要跟着翻）；其余模式断开
        if (bodyObs) { try { bodyObs.disconnect() } catch (e) {} bodyObs = null }
        if (themeMode === 'dsh' && typeof MutationObserver !== 'undefined' && document.body) {
          bodyObs = new MutationObserver(() => emit())
    ...
    function tagLabel(r) {
      ...
      if (!r.lineStart && r.kind === 'cite') return name + '(整文件)'
      return name
    }
    ...
    function openFilePicker() {
      ...
      callHost('get-root', store.sessionId ? { sessionId: store.sessionId } : {}).then((r) => {
    ...
    async function setWorkspaceRoot(path) {
      ...
      store.rootIsAuto = false // 用户显式选择：此后不再跟随会话切换
      store.rootSessionId = null
      await applyRoot((res && res.root) ? res.root : path)
      showToast('工作区根目录：' + store.rootPath)
    }
    // 换根并整体重置文件树/标签页/缓冲（自动跟随与手动设置共用）
    async function applyRoot(newRoot) {
    ...
    async function refresh() {
      ...
      // 自动根模式：首次无根、或会话已切换（rootSessionId 落后）都重新解析当前会话的工作区
      if (!store.rootPath || (store.rootIsAuto && sid && sid !== store.rootSessionId)) {
        const r = await callHost('get-root', sid ? { sessionId: sid } : {})
        const root = (r && r.root) || null
        if (!root) { store.error = '未找到工作区根目录'; emit(); return }
        await applyRoot(root)
        store.rootSessionId = sid
      } else {
        // 修复：刷新不再折叠整棵树，只重载根目录（子目录展开状态保留在 store.nodes）
        await reloadDir(store.rootPath)
      }
    }
    ...
    function onEditorContext(e) {
      ...
      const sel = { text, lineStart, lineEnd, path: store.activePath, language: langOf(store.activePath) }
      store.ctxMenu = { x: e.clientX, y: e.clientY, items: [
    ...
    async function reloadDir(path) {
      ...
      if (res && res.error) { store.error = res.error; emit(); return }
      const entries = (res && res.entries) || []
      n.children = entries.map((e) => { store.nodes[e.path] = e; return e.path })
      emit()
    }
    // 整文件引用：只给 AI 绝对路径，AI 用自己的读取工具打开
    async function citeWholeFile(path) {
      const ref = await callHost('add-reference', { path: path, language: langOf(path), lineStart: null, lineEnd: null, text: '', kind: 'cite' })
      if (ref && ref.reference) { store.references.push(ref.reference); emit() }
      showToast('已引用整个文件：' + baseName(path) + '（AI 将自行读取）')
    }
    // 重命名后同步标签页/缓冲/激活路径/watcher
    function renameTabSync(fromPath, toPath) {
      const affected = store.tabs.filter((t) => t.path === fromPath)
      affected.forEach((t) => { t.path = toPath; t.name = baseName(toPath) })
      if (toPath in store.buffers === false && fromPath in store.buffers) {
        store.buffers[toPath] = store.buffers[fromPath]
        store.savedContent[toPath] = store.savedContent[fromPath]
        store.dirty[toPath] = store.dirty[fromPath]
        store.externalChanged[toPath] = false
      }
      delete store.buffers[fromPath]; delete store.savedContent[fromPath]; delete store.dirty[fromPath]; delete store.externalChanged[fromPath]
      if (store.activePath === fromPath) store.activePath = toPath
      callHost('unwatch-file', { path: fromPath }).then(() => {})
      if (affected.length > 0) callHost('watch-file', { path: toPath }).then(() => {})
    }
    // 删除后清理：关掉该路径（及其目录下）所有打开的标签
    function closeTabsUnder(path) {
      const sep = String(path).includes('\\') ? '\\' : '/'
      const prefix = String(path).replace(/[\\/]+$/, '') + sep
      store.tabs.filter((t) => t.path === path || t.path.indexOf(prefix) === 0)
        .map((t) => t.path).forEach((p) => closeTab(p))
    }
    async function doRename(path, isDir) {
      const parent = parentPath(path)
      store.nameDialog = { mode: 'rename', path: path, dir: parent, isDir: !!isDir, curName: baseName(path) }
      emit()
    }
    async function doDelete(path, isDir) {
      store.confirmBox = {
        msg: isDir
          ? '确定删除文件夹「' + baseName(path) + '」吗？\n其中所有内容（含子目录）都会被永久删除。'
          : '确定删除文件「' + baseName(path) + '」吗？',
        danger: true,
        onOk: async () => {
          const res = await callHost('fs-delete', { path: path, recursive: !!isDir })
          if (res && res.error) { showToast('删除失败：' + res.error); return }
          closeTabsUnder(path)
          const parent = parentPath(path)
          if (parent && store.nodes[parent]) await reloadDir(parent)
          else await refresh()
    ...
    function openNodeMenu(e, node) {
      ...
      const items = []
      if (node.isDir) {
        items.push({ icon: '📄', label: '新建文件', act: () => { store.nameDialog = { mode: 'newfile', dir: node.path }; emit() } })
        items.push({ icon: '🗂', label: '新建文件夹', act: () => { store.nameDialog = { mode: 'newdir', dir: node.path }; emit() } })
        items.push({ icon: '🏠', label: '以此文件夹为根', act: () => setWorkspaceRoot(node.path) })
        items.push({ icon: '⟳', label: '刷新', act: () => reloadDir(node.path) })
      } else {
        items.push({ icon: '📄', label: '打开', act: () => openFile(node.path) })
        items.push({ icon: '🔗', label: '引用给 AI（整个文件）', act: () => citeWholeFile(node.path) })
      }
      items.push({ icon: '📋', label: '复制路径', act: () => { copyText(node.path); showToast('已复制路径：' + node.path) } })
      items.push({ icon: '📂', label: '在资源管理器中显示', act: () => callHost('fs-reveal', { path: node.path }).then((r) => { if (r && r.error) showToast('失败：' + r.error) }) })
      items.push({ icon: '✏️', label: '重命名', act: () => doRename(node.path, node.isDir) })
      const isRoot = node.path === store.rootPath
      items.push({ icon: '🗑', label: isRoot ? '删除（工作区根）' : (node.isDir ? '删除文件夹…' : '删除'), act: () => {
        if (isRoot) { showToast('工作区根目录不能在树里删除；请先更换根目录'); return }
        doDelete(node.path, node.isDir)
      } })
      store.ctxMenu = { x: e.clientX, y: e.clientY, items: items }
      emit()
    }
    // 文件树空白区右键
    function openTreeBlankMenu(e) {
      e.preventDefault()
      if (!store.rootPath) return
      store.ctxMenu = { x: e.clientX, y: e.clientY, items: [
        { icon: '📄', label: '新建文件（根目录）', act: () => { store.nameDialog = { mode: 'newfile', dir: store.rootPath }; emit() } },
        { icon: '🗂', label: '新建文件夹（根目录）', act: () => { store.nameDialog = { mode: 'newdir', dir: store.rootPath }; emit() } },
        { icon: '⟳', label: '刷新', act: () => refresh() },
        { icon: '🗃', label: '更换工作区根', act: () => openFilePicker() },
      ] }
      emit()
    }
    ...
    function buildSendPayload(draft) {
      ...
      // 整文件引用（无行号）：直接给出绝对路径，AI 用自己的读取工具打开。
      const text = draft || ''
      if (text.indexOf('[code UI] 已标记') >= 0) return text // 幂等：已带指针不重复附加
      const citeRefs = store.references.filter((r) => r.kind === 'cite')
      const trRefs = store.references.filter((r) => r.kind === 'translate')
      const codeRefs = citeRefs.filter((r) => r.lineStart)
      const fileRefs = citeRefs.filter((r) => !r.lineStart)
      const parts = []
      if (codeRefs.length > 0) parts.push(codeRefs.length + ' 段代码引用')
      if (fileRefs.length > 0) parts.push(fileRefs.length + ' 个文件引用')
      if (trRefs.length > 0) parts.push(trRefs.length + ' 条翻译请求')
      if (parts.length === 0) return text
      let ptr = '[code UI] 已标记 ' + parts.join('、') + '。'
      if (fileRefs.length > 0) {
        ptr += '\n文件引用（整个文件，请用你的文件读取工具打开以下绝对路径）：\n'
          + fileRefs.map((r) => '- ' + r.path).join('\n')
      }
      if (codeRefs.length > 0 || trRefs.length > 0) {
        ptr += '\n（代码/翻译引用内容未粘贴在消息里，请调用 get_referenced_snippets 工具读取）。'
      }
      const sep = text.trim() ? '\n\n' : ''
      return text + sep + ptr
    }
    ...
    /* ============ §7 组件层（V.* 命名空间） ============ */
    let selfSending = false // AiInput 副本自己发送时置位，让挂钩的 submit 不重复附加指针
    function InputBridge(props) {
      ...
      React.useEffect(() => {
        ...
        // 捕获当前会话 id：文件树默认根跟随当前会话的工作区目录
        if (props.sessionId && props.sessionId !== store.sessionId) {
          store.sessionId = props.sessionId
          if (store.rootIsAuto) {
            const hasDirty = Object.keys(store.dirty || {}).some((k) => store.dirty[k])
            if (hasDirty) showToast('会话已切换：有未保存文件，工作区暂不跟随（保存后点 ⟳ 刷新即可）')
            else refresh()
          }
        }
        emit()
      })
      // 挂钩 DSH 原生输入条的发送：存在引用时把指针附进草稿再发（整文件=绝对路径）。
      // actions 对象「每会话一个稳定身份」，补丁安全；卸载/换会话时还原原方法。
      React.useEffect(() => {
        if (!inputActions || typeof inputActions.submit !== 'function') return
        if (inputActions.__cdexPatched) return
        const orig = inputActions.submit
        inputActions.__cdexPatched = true
        inputActions.submit = function () {
          try {
            const hasRefs = store.references.length > 0
            const draft = store.inputDraft || ''
            if (hasRefs && !selfSending && draft.indexOf('[code UI]') < 0) {
              inputActions.setDraft(buildSendPayload(draft))
            }
          } catch (e) {}
          return orig.apply(inputActions, arguments)
        }
        return () => {
          try { inputActions.submit = orig; delete inputActions.__cdexPatched } catch (e) {}
        }
      }, [inputActions])
      return null
    }
      ...
      const goTo = (e) => {
        if (e.key === 'Enter') {
          ...
          callHost('stat-path', { path: p }).then((res) => {
            ...
            // 手动输入 + 回车 = 直接认定：目录设为工作区根，文件直接打开
            if (res.isDir) { setWorkspaceRoot(res.path || p) }
      ...
      return h('div', { className: 'cdex-modal', onClick: close },
        h('div', { className: 'cdex-modal-box', onClick: (e) => e.stopPropagation() },
          ...
          h('div', { className: 'cdex-fp-bar' },
            ...
            h('input', { className: 'cdex-fp-input', defaultValue: curDir || '', placeholder: '输入路径后回车：文件夹=设为根，文件=打开', onKeyDown: goTo }),
          ),
          ...
          h('div', { className: 'cdex-fp-actions' },
            h('span', { className: 'cdex-fp-hint' }, '输入路径回车即认定：文件夹设为根、文件直接打开；点条目仅浏览，此时用右下按钮确认'),
    ...
    function ModeTabs() {
      ...
      return h('div', { className: 'cdex-modebar ' + themeClass() + ' ' + tokensClass(), style: style, onPointerDown: onBarPointerDown },
        h('span', { className: 'cdex-mode-grip', title: '拖拽移动' }, '⠿'),
        h('button', { className: 'cdex-mode-tab' + (st.open ? ' active' : ''), onClick: () => { if (!st.open) toggleUiMode('codeui') } }, 'code UI'),
        h('button', { className: 'cdex-mode-tab' + (!st.open ? ' active' : ''), onClick: () => { if (st.open) toggleUiMode('native') } }, 'dsh UI'),
      )
    }
    function ActivityBar() {
      const st = useStore()
      return h('div', { className: 'cdex-activitybar' },
        h('button', { className: 'cdex-abar-btn' + (st.view === 'files' ? ' active' : ''), title: st.sidebarCollapsed ? '展开资源管理器' : '资源管理器（再点一次收起）', onClick: () => { if (st.sidebarCollapsed) { st.sidebarCollapsed = false; st.view = 'files' } else if (st.view === 'files') { st.sidebarCollapsed = true } else { st.view = 'files' } emit() } }, '📁'),
        ...
        h('button', { className: 'cdex-abar-btn', title: '退出编辑器（切回dsh UI）', onClick: () => toggleUiMode('native') }, '↩'),
      )
    }
    function TreeNode(props) {
      ...
      return h('div', null,
        h('div', { className: cls, style: style, onClick: () => { node.isDir ? toggleExpand(node.path) : openFile(node.path) }, onContextMenu: (e) => openNodeMenu(e, node) },
    ...
    function FileTree() {
      const st = useStore()
      return h('div', { className: 'cdex-sidebar', style: { width: st.sidebarW } },
        h('div', { className: 'cdex-sb-resize', title: '拖拽调整宽度（拖到最窄松手即收起）', onPointerDown: onSbResizeDown }),
        h('div', { className: 'cdex-sidebar-head' },
          ...
          h('span', null,
            ...
            h('button', { className: 'cdex-btn-ghost', onClick: () => { store.sidebarCollapsed = true; emit() }, title: '收起侧栏' }, '«'),
          ),
        ),
        h('div', { className: 'cdex-tree', onContextMenu: (e) => { if (!e.target.closest || !e.target.closest('.cdex-node')) openTreeBlankMenu(e) } },
    ...
    function RefItem(r) {
      return h('div', { className: 'cdex-ref', key: r.id },
        h('div', { className: 'cdex-ref-head' },
          h('span', { className: 'cdex-ref-path' }, r.path, r.lineStart ? (' :' + r.lineStart + '-' + r.lineEnd) : (r.kind === 'cite' ? ' （整个文件）' : '')),
          h('button', { className: 'cdex-btn-ghost', onClick: () => delReference(r.id) }, '✕'),
        ),
        h('div', { className: 'cdex-ref-code' }, r.text ? r.text.slice(0, 500) : '（AI 将按绝对路径自行读取整个文件）'),
      )
    }
    ...
    function ContextMenu() {
      ...
      if (!m) return null
      const items = m.items || []
      // 视口边界自适应：菜单在点击点放不下时收进视口（底部/右侧溢出钳位），
      // 避免在最后一行右键时菜单被浏览器底边截断
      const ITEM_H = 33
      ...
      try {
        const vw = (typeof window !== 'undefined' && window.innerWidth) || 1200
        const vh = (typeof window !== 'undefined' && window.innerHeight) || 800
        if (my + estH > vh - 8) my = Math.max(8, vh - estH - 8)
        if (mx + estW > vw - 8) mx = Math.max(8, vw - estW - 8)
      } catch (e) {}
      return h('div', { className: 'cdex-menu-backdrop', onClick: closeMenu },
        h('div', { className: 'cdex-menu', style: { left: mx, top: my }, onClick: (e) => e.stopPropagation() },
          items.map((it, i) => h('div', { key: i, className: 'cdex-menu-item', onClick: () => { try { it.act() } finally { closeMenu() } } },
    ...
    // 新建文件/文件夹、重命名 的输入弹窗（Enter 确认 / Esc 取消）
    function NameDialog() {
      const st = useStore()
      const d = st.nameDialog
      if (!d) return null
      const close = () => { st.nameDialog = null; nameInputEl = null; emit() }
      const titles = { newfile: '新建文件', newdir: '新建文件夹', rename: d.isDir ? '重命名文件夹' : '重命名文件' }
      const confirm = async () => {
        const name = (nameInputEl && nameInputEl.value || '').trim()
        if (!name) { showToast('请输入名称'); return }
        if (name.indexOf('/') >= 0 || name.indexOf('\\') >= 0) { showToast('名称不能包含路径分隔符'); return }
        if (d.mode === 'rename') {
          const to = joinPath(d.dir, name)
          const res = await callHost('fs-rename', { from: d.path, to: to })
          if (res && res.error) { showToast('重命名失败：' + res.error); close(); return }
          renameTabSync(d.path, to)
          if (store.nodes[d.dir]) await reloadDir(d.dir)
          else await refresh()
          showToast('已重命名为：' + name)
        } else {
          const full = joinPath(d.dir, name)
          const res = await callHost(d.mode === 'newfile' ? 'fs-createfile' : 'fs-mkdir', { path: full })
          if (res && res.error) { showToast('创建失败：' + res.error); close(); return }
          if (store.nodes[d.dir]) await reloadDir(d.dir)
          else await refresh()
          if (d.mode === 'newfile') await openFile(full)
          showToast('已创建：' + name)
        }
        close()
      }
      ...
      return h('div', { className: 'cdex-modal', onClick: close },
        h('div', { className: 'cdex-modal-box', onClick: (e) => e.stopPropagation() },
          h('div', { className: 'cdex-modal-title' }, titles[d.mode] || '输入名称', h('span', { style: { color: '#9d9d9d', fontWeight: 400 } }, ' — ', d.mode === 'rename' ? d.dir : d.dir || '')),
          h('input', { className: 'cdex-input', ref: (el) => { nameInputEl = el }, defaultValue: d.curName || '', placeholder: d.mode === 'rename' ? '输入新名称' : (d.mode === 'newfile' ? '例如 utils.js' : '例如 components'), onKeyDown: (e) => { if (e.key === 'Enter') { e.preventDefault(); confirm() } if (e.key === 'Escape') { e.preventDefault(); close() } } }),
          h('div', { className: 'cdex-modal-actions' },
            h('button', { className: 'cdex-btn-ghost', onClick: close }, '取消'),
            h('button', { className: 'cdex-btn', onClick: confirm }, d.mode === 'rename' ? '重命名' : '创建'),
          ),
        ),
      )
    }
    ...
    function ConfirmBox() {
      const st = useStore()
      const d = st.confirmBox
      if (!d) return null
      const close = () => { st.confirmBox = null; emit() }
      const ok = async () => { const fn = d.onOk; close(); if (typeof fn === 'function') { try { await fn() } catch (e) {} } }
      return h('div', { className: 'cdex-modal', onClick: close },
        h('div', { className: 'cdex-modal-box', onClick: (e) => e.stopPropagation() },
          h('div', { className: 'cdex-modal-title' }, '⚠ 确认操作'),
          h('div', { className: 'cdex-confirm-msg' }, d.msg),
          h('div', { className: 'cdex-modal-actions' },
            h('button', { className: 'cdex-btn-ghost', onClick: close }, '取消'),
            h('button', { className: 'cdex-btn cdex-btn-danger', onClick: ok }, '确定'),
          ),
        ),
      )
    }
    ...
    function Workbench() {
      ...
      return h('div', { className: 'cdex-workbench ' + themeClass() + ' ' + tokensClass(), style: style },
        ...
        h('div', { className: 'cdex-main' },
          h(ActivityBar),
          st.sidebarCollapsed ? h('div', { className: 'cdex-sb-expand', title: '展开侧栏', onClick: () => { st.sidebarCollapsed = false; emit() } }, '»') : (st.view === 'files' ? h(FileTree) : h(ReferencesView)),
          h(Editor),
        ),
        h(AiInput),
        h(StatusBar),
        ...
        st.nameDialog ? h(NameDialog) : null,
        st.confirmBox ? h(ConfirmBox) : null,
        st.toast ? h(Toast) : null,
      )
    }

    /* ============ 右侧停靠模式（feature/native-layout） ============
       code UI 固定占右侧；DSH 壳层帧（[data-slot=root] 下带 grid-template-columns
       内联样式的 Grid 帧）加 margin-right 推挤，聊天区留在左侧并排。
       左缘手柄拖宽；会话头按钮开关。定位用语义属性 + 内联样式，不依赖哈希类名。 */
    function findShellFrame() {
      try {
        const root = document.querySelector('[data-slot=root]')
        return (root && root.querySelector('[style*="grid-template-columns"]')) || null
      } catch (e) { return null }
    }
    function onDockResizeDown(e) {
      e.preventDefault()
      const handle = e.currentTarget
      const sw = store.dockW
      const px = e.clientX
      try { handle.setPointerCapture(e.pointerId) } catch (err) {}
      function move(ev) {
        store.dockW = Math.max(340, Math.min(sw + (px - ev.clientX), vw - 420))
        emit()
      }
      function up() {
        handle.removeEventListener('pointermove', move)
    ...
    // 文件树右缘拖宽：拖到足够窄（<140px）松手 -> 整体收起（宽度还原为拖前值，下次展开不残留）
    function onSbResizeDown(e) {
      e.preventDefault()
      const handle = e.currentTarget
      const sw = store.sidebarW
      const px = e.clientX
      try { handle.setPointerCapture(e.pointerId) } catch (err) {}
      function move(ev) {
        store.sidebarW = Math.max(60, sw + (ev.clientX - px))
        emit()
      }
      function up() {
        handle.removeEventListener('pointermove', move)
        handle.removeEventListener('pointerup', up)
        handle.removeEventListener('pointercancel', up)
        if (store.sidebarW < 140) {
          store.sidebarCollapsed = true
          store.sidebarW = Math.max(180, sw)
        }
        emit()
      }
      handle.addEventListener('pointermove', move)
    ...
    function DockToggle() {
      const st = useStore()
      return h('button', {
        className: 'cdex-dock-toggle' + (st.dockOpen ? ' active' : ''),
        title: '打开 / 收起 code UI 编辑器',
        onClick: () => { toggleUiMode(st.dockOpen ? 'native' : 'codeui') },
      }, '⌨ code UI')
    }
    function DockPanel() {
      const st = useStore()
      ...
      // 推挤 DSH 壳层帧：开 = 按面板宽度加 margin-right；关 / 卸载 = 还原
      React.useEffect(() => {
        const frame = findShellFrame()
        if (frame) frame.style.marginRight = st.dockOpen ? st.dockW + 'px' : ''
        return () => { const f = findShellFrame(); if (f) f.style.marginRight = '' }
      }, [st.dockOpen, st.dockW])
      if (!st.dockOpen) return null
      return h('div', { className: 'cdex-dock ' + themeClass() + ' ' + tokensClass(), style: { width: st.dockW } },
        h('div', { className: 'cdex-dock-grip', title: '拖拽调整宽度', onPointerDown: onDockResizeDown }),
        h('div', { className: 'cdex-nativeview-top' },
          h('span', { className: 'cdex-top-title', title: st.rootPath || '' }, st.rootPath ? ('code UI · ' + baseName(st.rootPath)) : 'code UI'),
          h('div', { className: 'cdex-top-spacer' }),
          h('button', { className: 'cdex-btn-ghost', title: '打开文件 (Ctrl+P)', onClick: openFilePicker }, '📂'),
          h('button', { className: 'cdex-btn-ghost', title: '收起 code UI', onClick: () => toggleUiMode('native') }, '✕'),
        ),
        h('div', { className: 'cdex-main' },
          h(ActivityBar),
          st.sidebarCollapsed ? h('div', { className: 'cdex-sb-expand', title: '展开侧栏', onClick: () => { st.sidebarCollapsed = false; emit() } }, '»') : (st.view === 'files' ? h(FileTree) : h(ReferencesView)),
    ...
    let layoutMode = 'dock'    // dock(右侧停靠,默认) / float(悬浮窗口)：编辑器以哪种布局呈现
    let toggleStyle = 'bar'    // bar(悬浮模式条,默认) / header(会话头 ⌨ 按钮)：用哪种按钮切换界面
    function mountEditor() {
      ...
      // 切换按钮：按用户选择注册（与布局模式解耦，任意组合）
      if (toggleStyle === 'bar') {
        // 悬浮模式条：可拖动横条，两键切换 code UI / dsh UI
        disposers.push(slots.inject('shell.overlay', () => slots.register(
          { name: 'shell.overlay', id: 'cdex-mode-tabs', order: 5, label: '界面模式' },
          () => h(ModeTabs, null),
        )))
      } else {
        // 会话头 ⌨ 按钮：点击在 code UI / dsh UI 之间切换
        disposers.push(slots.inject('conversation.session.header.actions', () => slots.register(
          { name: 'conversation.session.header.actions', id: 'cdex-codeui-toggle', order: 9998, label: 'code UI' },
          () => h(DockToggle, null),
        )))
      }
      // 编辑器布局：按布局模式注册
      if (layoutMode === 'float') {
        // 悬浮窗口：可拖拽悬浮工作台（含自带 AI 输入框，悬浮时盖住 DSH 输入条）
        disposers.push(slots.inject('shell.overlay', () => slots.register(
          { name: 'shell.overlay', id: 'cdex-workbench', order: 10, label: '代码编辑器' },
          () => h(Workbench, null),
        )))
      } else {
        // 右侧停靠（默认）：固定右缘停靠面板（AI 输入用 DSH 原生输入条）
        disposers.push(slots.inject('shell.overlay', () => slots.register(
          { name: 'shell.overlay', id: 'cdex-dock', order: 10, label: 'code UI 停靠面板' },
          () => h(DockPanel, null),
        )))
      }
    ...
    // 卡片里切换布局 -> set-layout 成功后调用：卸载旧挂载、按新模式重挂（即时生效，无需刷新）
    function applyLayout(v) {
      const next = (v === 'float') ? 'float' : 'dock'
      if (next === layoutMode) return
      layoutMode = next
      if (unmountEditorFns) { unmountEditor(); mountEditor() }
    }
    // 卡片里切换切换按钮样式 -> set-toggle 成功后调用：换一种切换按钮（即时生效，无需刷新）
    function applyToggleStyle(v) {
      const next = (v === 'bar') ? 'bar' : 'header'
      if (next === toggleStyle) return
      toggleStyle = next
      if (unmountEditorFns) { unmountEditor(); mountEditor() }
    }
    // 界面模式（codeui=显示编辑器 / native=dsh UI）：统一驱动两种布局的显示状态。
    // applyUiMode 只改 store；toggleUiMode 供界面入口（模式条/⌨/✕/↩）用：改 store + 静默持久化。
    function applyUiMode(v) {
      const show = v !== 'native'
      store.open = show
      store.dockOpen = show
      emit()
    }
    function toggleUiMode(v) {
      applyUiMode(v)
      callHost('set-uimode', { uiMode: v }).catch(() => {})
    }
    ...
    // 启动时先查开关/主题/布局/界面模式/切换按钮再挂载（新装默认：启用+跟随DSH+停靠+隐藏+悬浮模式条；
    // RPC 失败回退到「可见+会话头按钮」保证可发现性）
    callHost('get-self').then((r) => {
      if (r && !r.error) {
        setThemeMode(r.theme)
        layoutMode = (r.layout === 'float') ? 'float' : 'dock'
        toggleStyle = (r.toggle === 'header') ? 'header' : 'bar'
        applyUiMode(r.uiMode === 'codeui' ? 'codeui' : 'native')
        applyEnabled(!(r.enabled === false))
      } else {
        setThemeMode('dsh')
        layoutMode = 'dock'
        toggleStyle = 'header'
    ...
    }).catch(() => { setThemeMode('dsh'); layoutMode = 'dock'; toggleStyle = 'header'; applyUiMode('codeui'); applyEnabled(true) })
    ...
    function PluginConfigCard() {
      ...
      const [uishing, setUishing] = React.useState(false)
      const [toggling, setToggling] = React.useState(false)
      const lastPayloadRef = React.useRef(null)
      const pollDisposeRef = React.useRef(null)

      // 编辑器主题：写 host settings 持久化；成功后立即换肤
      function setTheme(m) {
        if (!self || theming) return
        setTheming(true)
        callHost('set-theme', { theme: m }).then((r) => {
          setTheming(false)
          if (r && !r.error) {
            setSelf(Object.assign({}, self, { theme: m }))
            setThemeMode(m)
            showToast('主题已切换：' + (m === 'system' ? '跟随系统' : m === 'light' ? '浅色' : '深色'))
          } else {
      ...
      // 编辑器界面（三态，合并原「布局模式 × 界面模式」）：
      // dock=右侧停靠 / float=悬浮窗口 / hidden=隐藏(dsh UI，内部记住当前布局，切回原样恢复)
      function setUiShape(v) {
        if (!self || uishing) return
        const curLayout = (self.layout === 'float') ? 'float' : 'dock'
        const curHidden = self.uiMode === 'native'
        if (v === 'hidden') {
          if (curHidden) return
          setUishing(true)
          callHost('set-uimode', { uiMode: 'native' }).then((r) => {
            setUishing(false)
            if (r && !r.error) {
              setSelf(Object.assign({}, self, { uiMode: 'native' }))
              applyUiMode('native')
              showToast('编辑器已隐藏（dsh UI）')
            } else { showToast('切换失败：' + ((r && r.error) || '未知错误')) }
          })
          return
        }
        const needLayout = curLayout !== v
        const needShow = curHidden
        if (!needLayout && !needShow) return
        setUishing(true)
        const next = Object.assign({}, self, { layout: v, uiMode: 'codeui' })
        const step = () => {
          callHost('set-uimode', { uiMode: 'codeui' }).then((r2) => {
            setUishing(false)
            if (r2 && !r2.error) {
              setSelf(next)
              applyUiMode('codeui')
              showToast('编辑器界面：' + (v === 'dock' ? '右侧停靠' : '悬浮窗口'))
            } else { showToast('切换失败：' + ((r2 && r2.error) || '未知错误')) }
          })
        }
        if (needLayout) {
          callHost('set-layout', { layout: v }).then((r1) => {
            if (r1 && !r1.error) { applyLayout(v); step() }
            else { setUishing(false); showToast('切换失败：' + ((r1 && r1.error) || '未知错误')) }
          })
        } else { step() }
      }

      // 切换按钮样式：写 host settings 持久化；成功后立即换一种切换按钮
      function setToggle(m) {
        if (!self || toggling) return
        setToggling(true)
        callHost('set-toggle', { toggle: m }).then((r) => {
          setToggling(false)
          if (r && !r.error) {
            setSelf(Object.assign({}, self, { toggle: m }))
            applyToggleStyle(m)
            showToast('切换按钮已改为：' + (m === 'bar' ? '悬浮模式条' : '会话头按钮'))
          } else {
            showToast('切换失败：' + ((r && r.error) || '未知错误'))
          }
        })
      ...
      // 更新已成功、等待重启：更新类按钮文案临时改为「需重启 DSH 生效」，用户一眼可见
      const updated = !!(run && run.done && run.ok)
      let checkLine = null
      ...
      return h('li', { className: 'cdx-pc-card' + (cardOpen ? ' cdx-pc-card-open' : '') },
        ...
        cardOpen ? h('div', { className: 'cdx-pc-body' },
          ...
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              h('span', { className: 'cdx-pc-label' }, '编辑器主题'),
              h('span', { className: 'cdx-pc-head' },
                ['dsh', 'system', 'light', 'dark'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.theme) || 'dsh') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || theming,
                  onClick: () => setTheme(m),
                }, m === 'dsh' ? '跟随 DSH' : m === 'system' ? '跟随系统' : m === 'light' ? '浅色' : '深色')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '「跟随 DSH」随 DSH 整体外观（默认）；「跟随系统」随操作系统浅色/深色实时切换；浅色/深色为强制指定。切换立即生效，仅作用于编辑器界面，选择持久化保存。'),
          ),
          // 字段：编辑器界面（三态，合并原「布局模式 × 界面模式」）
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              h('span', { className: 'cdx-pc-label' }, '编辑器界面'),
              h('span', { className: 'cdx-pc-head' },
                (function () {
                  const curLayout = ((self && self.layout) === 'float') ? 'float' : 'dock'
                  const cur = (self && self.uiMode) === 'native' ? 'hidden' : curLayout
                  return ['dock', 'float', 'hidden'].map((m) => h('button', {
                    key: m,
                    type: 'button',
                    className: 'cdx-pc-btn ' + (cur === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                    disabled: !self || uishing,
                    onClick: () => setUiShape(m),
                  }, m === 'dock' ? '右侧停靠' : m === 'float' ? '悬浮窗口' : '隐藏（dsh UI）'))
                })(),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '「隐藏（dsh UI）」为默认：新装不自动占屏，⌨ / 模式条 / ✕ 随时唤回，隐藏期间记住当前布局、切回原样恢复；「右侧停靠」code UI 固定占屏幕右侧、与聊天并排（默认布局）；「悬浮窗口」为可拖拽的浮动编辑器窗口、含自带输入框。切换即时生效，持久化保存。'),
          ),
          // 字段：切换按钮
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              h('span', { className: 'cdx-pc-label' }, '切换按钮'),
              h('span', { className: 'cdx-pc-head' },
                ['header', 'bar'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.toggle) || 'bar') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || toggling,
                  onClick: () => setToggle(m),
                }, m === 'header' ? '会话头按钮' : '悬浮模式条')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '选择用哪种按钮在 code UI 与 dsh UI 之间切换：「悬浮模式条」为屏幕上方可拖动的横条（code UI / dsh UI 两键，默认）；「会话头按钮」在会话标题栏显示 ⌨ code UI。与布局模式无关，可任意组合，选择持久化保存。'),
          ),
          // 字段一：在线检查更新
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
          ...
          chk && chk.updateAvailable ? h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              ...
              h('button', { type: 'button', className: 'cdx-pc-btn ' + (updated ? 'cdx-pc-btn-outline' : 'cdx-pc-btn-primary'), disabled: started || updated, onClick: () => startUpdate({ source: 'registry', version: chk.latest }) }, updated ? '需重启 DSH 生效' : '一键更新到 v' + chk.latest),
            ),
          ) : null,
          // 字段三：本地包更新
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              ...
              h('button', { type: 'button', className: 'cdx-pc-btn ' + (updated ? 'cdx-pc-btn-outline' : 'cdx-pc-btn-outline'), disabled: started || updated || !localPath.trim(), onClick: () => startUpdate({ source: 'local', path: localPath.trim() }) }, updated ? '需重启 DSH 生效' : '安装本地包'),
            ),
            h('div', { className: 'cdx-pc-head' },
              h('input', {
