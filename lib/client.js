...
// --- begin original apply body ---
    ...
    function ModeTabs() {
      ...
      return h('div', { className: 'cdex-modebar ' + themeClass() + ' ' + tokensClass(), style: style, onPointerDown: onBarPointerDown },
        ...
        h('button', { className: 'cdex-mode-tab' + (!st.open ? ' active' : ''), onClick: () => { if (st.open) toggleUiMode('native') } }, '原始风格-ui'),
      )
    }
    function ActivityBar() {
      ...
      return h('div', { className: 'cdex-activitybar' },
        ...
        h('button', { className: 'cdex-abar-btn', title: '退出编辑器（切回原始风格-ui）', onClick: () => toggleUiMode('native') }, '↩'),
      )
    }
    ...
    // 界面模式（codeui=显示编辑器 / native=原始风格-ui）：统一驱动两种布局的显示状态。
      ...
      function setUiMode(m) {
        ...
        callHost('set-uimode', { uiMode: m }).then((r) => {
          ...
          if (r && !r.error) {
            ...
            applyUiMode(m)
            showToast('界面已切换：' + (m === 'codeui' ? 'Code UI' : '原始风格-ui'))
          } else {
            showToast('切换失败：' + ((r && r.error) || '未知错误'))
          }
        })
            ...
            h('div', { className: 'cdx-pc-head' },
              ...
              h('span', { className: 'cdx-pc-head' },
                ['codeui', 'native'].map((m) => h('button', {
                  ...
                  onClick: () => setUiMode(m),
                }, m === 'codeui' ? 'Code UI' : '原始风格-ui')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '「Code UI」显示编辑器界面（默认）；「原始风格-ui」隐藏编辑器、回归 DSH 原生界面（入口按钮保留，随时切回）。悬浮模式条 / ⌨ / ✕ / ↩ 的每次切换都会同步到此处，选择持久化保存。'),
          ),
          // 字段：切换按钮
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
            ...
            h('p', { className: 'cdx-pc-note' }, '选择用哪种按钮在 Code UI 与原始风格-ui 之间切换：「会话头按钮」在会话标题栏显示 ⌨ Code UI（默认）；「悬浮模式条」为屏幕上方可拖动的横条（Code UI / 原始风格-ui 两键）。与布局模式无关，可任意组合，选择持久化保存。'),
          ),
          // 字段一：在线检查更新
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
