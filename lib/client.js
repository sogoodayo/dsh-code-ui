...
// --- begin original apply body ---
    ...
    let layoutMode = 'dock' // dock(右侧停靠,默认) / float(悬浮窗口)
    function mountEditor() {
      ...
      if (layoutMode === 'float') {
        // 悬浮窗口模式：悬浮模式条 + 可拖拽悬浮工作台（含自带 AI 输入框，悬浮时盖住 DSH 输入条）
        ...
        // 右侧停靠模式（默认）：会话头开关按钮 + 固定右缘停靠面板（AI 输入用 DSH 原生输入条）
        disposers.push(slots.inject('conversation.session.header.actions', () => slots.register(
    ...
    // 启动时先查开关/主题/布局/界面模式再挂载（RPC 失败 = 默认启用 + 跟随 DSH + 右侧停靠 + 显示编辑器）
    ...
    }).catch(() => { setThemeMode('dsh'); layoutMode = 'dock'; applyUiMode('codeui'); applyEnabled(true) })
