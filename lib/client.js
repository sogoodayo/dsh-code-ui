...
// --- begin original apply body ---
    ...
    /* ============ 编辑器主题三模式：system(跟随 OS) / light / dark ============ */
    // 原理：编辑器根节点挂 .cdex-theme-light / .cdex-theme-dark 类，类内局部重定义
    // --dsw-alias-* 语义变量（CSS 自定义属性作用域），不影响 DSH 其他界面。
    ...
    function themeClass() {
      ...
      return dark ? 'cdex-theme-dark' : 'cdex-theme-light'
    }
    function setThemeMode(v) {
      themeMode = (v === 'light' || v === 'dark') ? v : 'system'
      try {
    ...
    // 启动时先查开关与主题再挂载（RPC 失败 = 默认启用 + 跟随系统）
    callHost('get-self').then((r) => {
      if (r && !r.error) {
        if (r.theme === 'light' || r.theme === 'dark') setThemeMode(r.theme)
    ...
    }).catch(() => { setThemeMode('system'); applyEnabled(true) })
          ...
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
              ...
              h('span', { className: 'cdx-pc-head' },
                ['system', 'light', 'dark'].map((m) => h('button', {
                  key: m,
                  type: 'button',
                  className: 'cdx-pc-btn ' + (((self && self.theme) || 'system') === m ? 'cdx-pc-btn-primary' : 'cdx-pc-btn-outline'),
                  disabled: !self || theming,
                  onClick: () => setTheme(m),
                }, m === 'system' ? '跟随系统' : m === 'light' ? '浅色' : '深色')),
              ),
            ),
            h('p', { className: 'cdx-pc-note' }, '切换立即生效，仅作用于编辑器界面；「跟随系统」随操作系统浅色/深色外观实时切换，选择持久化保存。'),
          ),
          // 字段一：在线检查更新
          h('div', { className: 'cdx-pc-field' },
            h('div', { className: 'cdx-pc-head' },
